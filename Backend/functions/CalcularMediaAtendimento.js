...
async function calcularMediaTempo(data, totemLocation) {
  var DBtable;
  ...
  const sql = `
    SELECT AVG(TIMESTAMPDIFF(SECOND, hora_registro_gerado, hora_registro_atendimento)) AS media_segundos
    FROM ${DBtable}
  ...
  try {
    ...
    if (rows.length > 0) {
      ...
      return { success: true, media_minutos: mediaMinutos };
    } else {
...
module.exports = calcularMediaTempo;
